...
const ServiceComponent: React.FC<ServiceComponentProps> = ({ ServiceTitle, ServicDescription, ServiceImage }) => {
  const limitedWords =
    ServicDescription.split(' ').slice(0, 20).join(' ') +
    (ServicDescription.split(' ').length > 20 ? '...' : '');
